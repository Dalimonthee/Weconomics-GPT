...
async def extract_entities(
    ...
    # Load entity types from environment or use default generalized types
    env_entity_types = os.environ.get("DEFAULT_ENTITY_TYPES", "")
    if env_entity_types:
        try:
            # Parse the environment variable if it's in a list-like format
            import ast
            entity_types = ast.literal_eval(env_entity_types)
            logger.info(f"Using entity types from environment: {entity_types}")
        except (SyntaxError, ValueError):
            # If parsing fails, use comma-separated format
            entity_types = [t.strip() for t in env_entity_types.split(",") if t.strip()]
            logger.info(f"Using comma-separated entity types from environment: {entity_types}")
    else:
        # Use generalized entity types if not specified in environment
        entity_types = [
        ...
        logger.info(f"Using default generalized entity types: {entity_types}")
    
    # Override with config if available
    config_entity_types = global_config["addon_params"].get("entity_types")
    if config_entity_types:
        entity_types = config_entity_types
        logger.info(f"Overriding with entity types from config: {entity_types}")
    
    # Check if we should disable examples in production
    disable_examples = global_config["addon_params"].get("disable_examples", False)
    # Also check environment variables directly as a fallback
    if not disable_examples and os.environ.get("DISABLE_EXAMPLES", "").lower() == "true":
        disable_examples = True
        logger.info("Examples disabled by environment variable")

    # Always disable examples for general entity types to avoid hallucination
    if len(entity_types) > 5:
        disable_examples = True
        logger.info("Examples disabled automatically due to general entity types")

    # Initialize base context for prompt formatting
    context_base = {
        "language": language,
    ...
    # The base prompt for entity extraction
    entity_extract_prompt = PROMPTS["entity_extraction"]

    # Add examples if not disabled
    if not disable_examples:
        ...
        examples_section = ""
        logger.info("Examples are disabled for extraction")

    context_base["examples_section"] = examples_section

    # Create prompts for additional extraction steps
    continue_prompt = PROMPTS["entity_continue_extraction"].format(**context_base)
    if_loop_prompt = PROMPTS["entity_if_loop_extraction"].format(**context_base)
    ...
    # Log start message
    log_message = f"Starting entity extraction on {total_chunks} chunks with {len(entity_types)} entity types"
    ...
    async def _process_extraction_result(
        ...
        if entity_names:
            logger.debug(f"Extracted entities from chunk {chunk_key}: {entity_names}")
        
        return maybe_nodes, maybe_edges

    # Function to validate and ensure all placeholders are correctly replaced
    def validate_prompt(prompt_text):
        """Check if any placeholders remain in the prompt text"""
        placeholder_pattern = re.compile(r'\{([^{}]+)\}')
        remaining_placeholders = placeholder_pattern.findall(prompt_text)
        
        if remaining_placeholders:
            # Log warning and list all unreplaced placeholders
            logger.warning(f"Unreplaced placeholders in prompt: {remaining_placeholders}")
            # Replace unreplaced placeholders with empty strings to prevent errors
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        # Prepare the content with focus on domain-specific entities
        enhanced_content = f"""
Please extract entities with a focus on:
- Specific named entities (people, organizations, technologies, systems)
- Domain concepts and terminology
- Components, functions, and mechanisms mentioned
- Standards, policies, and protocols described

TEXT CONTENT:
{content}
"""

        # Get initial extraction - Format the prompt twice to ensure all placeholders are replaced
        hint_prompt = entity_extract_prompt.format(
            **context_base, input_text="{input_text}"
        ).format(**context_base, input_text=enhanced_content)
        
        # Validate the prompt to ensure all placeholders are replaced
        hint_prompt = validate_prompt(hint_prompt)
        ...
        # If no entities were found on first pass, try with a more direct prompt
        if not maybe_nodes and entity_extract_max_gleaning > 0:
            logger.info(f"No entities found in chunk {chunk_key}, trying with enhanced prompt")
            
            entity_reminder = f"""
IMPORTANT: The text might contain domain-specific entities that weren't extracted in the first pass.
Look for the following types of entities: {context_base['entity_types']}
Each entity should have:
1. A clear name
2. A specific type from the list above
3. A comprehensive description

Also identify relationships between these entities.
"""
            continue_prompt_enhanced = PROMPTS["entity_continue_extraction"].format(**context_base) + entity_reminder
            validated_continue_prompt = validate_prompt(continue_prompt_enhanced)
            
            glean_result = await use_llm_func_with_cache(
                validated_continue_prompt,
                use_llm_func,
                llm_response_cache=llm_response_cache,
            ...
            history += pack_user_ass_to_openai_messages(validated_continue_prompt, glean_result)
            ...
            # Use the gleaning results as the initial ones if they found entities
            if glean_nodes:
                maybe_nodes = glean_nodes
                maybe_edges = glean_edges
                logger.info(f"Found {len(glean_nodes)} entities after enhancement for chunk {chunk_key}")

        # Process additional gleaning results (if any entities were found)
        if maybe_nodes or entity_extract_max_gleaning > 1:
            for now_glean_index in range(1, entity_extract_max_gleaning):
                # Validate the continue prompt
                validated_continue_prompt = validate_prompt(continue_prompt)
                ...
                # Validate the loop prompt
                validated_if_loop_prompt = validate_prompt(if_loop_prompt)
                
                if_loop_result: str = await use_llm_func_with_cache(
                    validated_if_loop_prompt,
                    use_llm_func,
                    llm_response_cache=llm_response_cache,
                    history_messages=history,
                    cache_type="extract",
...
async def kg_query(
    ...
    # Add knowledge graph to query_param for direct entity lookups
    query_param.knowledge_graph_inst = knowledge_graph_inst
...
async def extract_keywords_only(
    ...
    # Check if query is asking about a specific entity
    if await _is_entity_name_query(text, param.knowledge_graph_inst):
        return [], [text.strip("?. ")]
...
async def _is_entity_name_query(query_text: str, knowledge_graph_inst: BaseGraphStorage = None) -> bool:
    """
    Check if query is directly asking about a specific entity in the knowledge graph.
    This helps with queries like "Who is X?" or "What is X?" where X is an entity name.
    ...
    Returns:
        True if query appears to be asking about a specific entity that exists in the graph
    """
    # Skip if no knowledge graph is provided
    if knowledge_graph_inst is None:
        return False
        
    # Clean and normalize the query
    clean_query = query_text.strip().strip("?").strip()
    
    # Extract potential entity names by removing common question words
    ...
    for pattern in entity_query_patterns:
        ...
        if match:
            ...
            # Check if this potential entity exists in the knowledge graph
            try:
                # Try exact match first
                if await knowledge_graph_inst.has_node(potential_entity):
                    logger.info(f"Entity direct match found: {potential_entity}")
                    return True
                    
                # Try case-insensitive match
                node_labels = await knowledge_graph_inst.get_all_labels()
                for label in node_labels:
                    if label.lower() == potential_entity.lower():
                        logger.info(f"Entity case-insensitive match found: {label}")
                        return True
...
async def get_keywords_from_query(
    ...
    # If we have a knowledge graph, check if this is a direct entity query
    if hasattr(query_param, 'knowledge_graph_inst') and query_param.knowledge_graph_inst:
        if await _is_entity_name_query(query, query_param.knowledge_graph_inst):
            # For direct entity queries, use the entity name as a low-level keyword
            potential_entity = re.sub(r'^who is |^what is |^tell me about |^describe |^information about |^explain ', '', query.lower().strip('?. '))
            return [], [potential_entity.strip()]
